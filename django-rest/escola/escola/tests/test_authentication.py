...
class AuthenticationUserTestCase(APITestCase):
    ...
    def test_autenticacao_user_com_username_incorreto(self):
        ...
        usuario = authenticate(username = 'adminUsername',password='admin')
        self.assertFalse((usuario is not None) and (usuario.is_authenticated))
        
    def test_autenticacao_user_com_password_incorreta(self):
        '''
        Teste que verifica a autenticação de um user com a password incorreta
        '''
        
        usuario = authenticate(username = 'admin',password = '123456')
        self.assertFalse((usuario is not None) and (usuario.is_authenticated))
